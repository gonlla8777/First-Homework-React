    ...
    if (children){
        return(
            ...
            <ListItem onClick = {()=> setOpen (status=> !status)}>
                <ListItemButton>
                    <ListItemText sx={{frontWeight:400}} primary={item.title} disableTypography/>
                </ListItemButton>
                        ...
                        children.map((child,index)=>{
                            return(
                                ...
                                onClick={()=>navigate(child.path)} disablePadding>
                                    <ListItemButton sx={{height:42, frontWeight:"200", padding: th=> th.spacing(0,2.5,0,3)}}>
                                        <ListItemIcon sx={{width:26, height:26, display:"flex", alignItems:"center", justifyContent:"center", padding:th=>th.spacing(0,2,0,0)}}>
                                            <Box component="span" sx={{width:4, height:4, display:"flex", borderRadius:"50%", alignItems:"center", justifyContent:"center", backgroundColor:"text.disabled"}} />
                                        </ListItemIcon>
                                        <ListItemText sx={{frontWeight:400}} primary= {child.title}/>
                                    </ListItemButton>
                                </ListItem>
                            )
                        })
    ...
    return (
        <>
            <ListItem onClick= {()=>navigate(path)}>
                <ListItemButton>
                    <ListItemText sx={{frontWeight:400}} primary={title} disableTypography />
                </ListItemButton>
            </ListItem>
        </>
    )
...
const MenuPopover = ({children, sx, ...other})=> {
    return(
        <Popover anchorOrigin={{vertical:"botton", horizontal:"right"}}
...
const PopMenu = () =>{
    ...
    return(
        <Box>
            ...
            anchorEl={menuRef.current} sx={{marginTop:2.5, marginLeft:0.5, overFlow:"inherit", boxShadow:"1px,1px,2px,2px rgb(0 0 0 / 20%)", width:320}}>
                {popMenu.map(item=>(
